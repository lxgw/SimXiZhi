...
> [!IMPORTANT]
> 自 1.100 版开始，只提供 GB 18030 字库版本，即原「新致宋体＋」「新晰黑体＋」改名为「新致宋体」「新晰黑体」，原 GBK 字符集「新致宋体」「新晰黑体」不再提供。
...
- 已有用户反馈，用本字体的硬替换版本替换系统字体后，Adobe Illustrator 等 Adobe 应用会显示乱码。如果您使用上述 Adobe 应用，请勿使用本字体替换系统默认字体。
...
- 如果您要将本字体替换回 IPA 原始授权字体，请从 [IPA 字体 Ver.003.03 下载页面](https://moji.or.jp/ipafont/ipa00303)获取原始授权字体「IPA ゴシック」和「IPA 明朝」。  
  对于涉及字体文件再分发的嵌入式应用（如应用程序、硬件设备、网页等嵌入），需要满足 IPA 许可中针对衍生字体再分发的限制条件，[请点击此处]([请点击此处](https://github.com/lxgw/lxgw/blob/main/documents/xizhi_embedding_instructions.md)了解。)了解。
